...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_rollout', type=int, default=3000)
    ...
    parser.add_argument('--dsize', type=int, default=1000000)
    parser.add_argument('--gamma', type=float, default=0.99)
    parser.add_argument('--quantile', type=float, default=0.1)
    args = parser.parse_args()
    # set random seed
    # torch.manual_seed(args.gpu_idx)
    # np.random.seed(args.gpu_idx)
    # random.seed(args.gpu_idx)
    ...
    file_name = f"TD3_{args.env_name}_0_1.0_forward"
    buffer = np.load(f'Dataset/{file_name}.npy', allow_pickle=True).item()
    ...
    file_name2 = f"TD3_{args.env_name}_0_1.0"
    buffer2 = np.load(f'Dataset/{file_name}.npy', allow_pickle=True).item()
